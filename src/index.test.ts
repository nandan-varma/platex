...
describe('compile() - local pipeline routing', () => {
  it(
    'compiles locally (via bundled Tectonic) when no serviceUrl is given',
    async () => {
...
describe('compile() - remote routing (full HTTP round trip)', () => {
  ...
  beforeAll(async () => {
    ...
    server = await new Promise<ServerType>((resolve) => {
      const s = serve({ fetch: app.fetch, port: 0 }, (_info) => resolve(s));
    });
  ...
  it(
    'compiles through a real platex HTTP service end-to-end',
    async () => {
      const source = await readFixture('minimal.tex');
  ...
  it(
    'surfaces compile errors returned by the remote service',
    async () => {
  ...
  it(
    'throws when the remote service is unreachable',
    async () => {
      await expect(
